...
static  void neumann_point_propagate(
        Front *front,
        POINTER wave,  // void pointer
        POINT *oldp,
        POINT *newp,
        HYPER_SURF_ELEMENT *oldhse, // 3D is triangle; 2D is segment
        HYPER_SURF         *oldhs,  // 3D is surface; 2D is curve
        double              dt,
        double              *V)
{
        int i, dim = front->rect_grid->dim; // rectangular not the dual but all Cartesian
	double nor[MAXD];  // normal vector
        double p1[MAXD], p2[MAXD]; // two points
        double *p0 = Coords(oldp); // coords of old point
        double dn, *h = front->rect_grid->h; // grid size
        double s0, s1, s2; // state variables
        STATE *sl, *sr, *state; // state struct variables
	CRT_PARAMS *cRparams = (CRT_PARAMS*)front->extra2; // access x-tal params through FronTier
        double *solute = cRparams->field->solute;

        for (i = 0; i < dim; ++i)
            Coords(newp)[i] = Coords(oldp)[i];

       	FT_GetStatesAtPoint( oldp, oldhse, oldhs, (POINTER*)&sl, (POINTER*)&sr ); // point on interface has two states; left and right

        // if true, pick left state; false pick right state
       	state = (negative_component(oldhs) == SOLUTE_COMP) ? sl : sr;

        // state of point on the front
	s0 = state->solute;

       	FT_NormalAtPoint( oldp, front, nor, SOLUTE_COMP ); // get normal pointing into the solute

       	dn = grid_size_in_direction( nor, h, dim ); // displacement along the normal

        // compute position of point p1 from p0 into solute
       	for (i = 0; i < dim; ++i)
       	    p1[i] = p0[i] + nor[i]*dn;

        // bilinear interpolation of state on point p1; s1
	FT_IntrpStateVarAtCoords( front, SOLUTE_COMP, p1, solute,
			          getStateSolute, &s1, &s0);

        // compute position of point p2 from p1 into the solute
       	for (i = 0; i < dim; ++i)
       	    p2[i] = p1[i] + nor[i]*dn;

        // bilinear interpolation of state on point p2; s2
	FT_IntrpStateVarAtCoords( front, SOLUTE_COMP, p2, solute,
			          getStateSolute, &s2, &s1);

        // find state of front point
	state = (negative_component(oldhs) == SOLUTE_COMP) ? 
		(STATE*)left_state(newp) : (STATE*)right_state(newp);

        // find state value at front point as weighted value of s1 and s2
        // second order extrapolation ?? s2->s1->s0
